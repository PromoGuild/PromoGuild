...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PGDMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PGDMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PGDMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("promoguild-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("PGDMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running PGDMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadPGDMiner() exception");
    } catch (...) {
        LogPrintf("ThreadPGDMiner() exception");
    }

    LogPrintf("ThreadPGDMiner exiting\n");
}
